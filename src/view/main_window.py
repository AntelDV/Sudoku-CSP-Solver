...
class MainView(ctk.CTkFrame):
    ...
    def kiem_tra_nhap_lieu(self, gia_tri_moi):
        # Cho phép nhập nhiều chữ số (để hỗ trợ size 16x16, 25x25)
    ...
    def rebuild_grid(self, n):
        """Vẽ lại toàn bộ lưới dựa trên kích thước N."""
        self.current_n = n
        
        # Xóa nội dung cũ trong khung lưới
    ...
    def tao_khung_numpad(self, parent):
        self.frame_numpad = ctk.CTkFrame(parent, fg_color="transparent")
        
        label = ctk.CTkLabel(self.frame_numpad, text="Bàn phím nhập liệu:", font=ctk.CTkFont(size=14, weight="bold"))
        label.pack(pady=(0, 2))
        ...
        for num, r, c in btn_nums:
            btn = ctk.CTkButton(
                grid_frame, text=str(num), width=50, height=40,
                font=ctk.CTkFont(size=16, weight="bold"),
                command=lambda n=num: self.controller.handle_numpad_click(n)
            )
        ...
        btn_clear.grid(row=3, column=1, padx=3, pady=3)
    ...
    def tao_khung_nap_de(self, parent):
        ...
        self.btn_csv_easy = ctk.CTkButton(khung_kho, text="Lấy Đề Dễ", font=ctk.CTkFont(size=12, weight="bold"), fg_color="#28a745", hover_color="#32CD32", text_color="#FFFFFF", height=28)
        self.btn_csv_easy.grid(row=0, column=0, padx=(0, 3), pady=2)
        self.btn_csv_medium = ctk.CTkButton(khung_kho, text="Lấy Đề Trung Bình", font=ctk.CTkFont(size=12, weight="bold"), fg_color="#FFC107", hover_color="#FFD700", text_color="#000000", height=28)
        self.btn_csv_medium.grid(row=0, column=1, padx=(3, 0), pady=2)
        self.btn_csv_hard = ctk.CTkButton(khung_kho, text="Lấy Đề Khó", font=ctk.CTkFont(size=12, weight="bold"), fg_color="#E74C3C", hover_color="#EC7063", text_color="#FFFFFF", height=28)
        self.btn_csv_hard.grid(row=1, column=0, padx=(0, 3), pady=2)
        self.btn_csv_extreme = ctk.CTkButton(khung_kho, text="Lấy Đề Siêu Khó", font=ctk.CTkFont(size=12, weight="bold"), fg_color="#8E44AD", hover_color="#A569BD", text_color="#FFFFFF", height=28)
    ...
    def tao_khung_hanh_dong(self, parent):
        ...
        self.btn_giai = ctk.CTkButton(khung_nut_hanh_dong, text="⚡ GIẢI", font=ctk.CTkFont(size=14, weight="bold"), fg_color="#28a745", hover_color="#32CD32", text_color="#FFFFFF", height=35, width=105)
        self.btn_giai.grid(row=0, column=0, padx=(0, 3))
        
        self.btn_sosanh = ctk.CTkButton(khung_nut_hanh_dong, text="📊 SO SÁNH", font=ctk.CTkFont(size=14, weight="bold"), fg_color="#8E44AD", hover_color="#A569BD", text_color="#FFFFFF", height=35, width=105)
        self.btn_sosanh.grid(row=0, column=1, padx=3)

        self.btn_xoa = ctk.CTkButton(khung_nut_hanh_dong, text="✕ XÓA", font=ctk.CTkFont(size=14, weight="bold"), fg_color="#E74C3C", hover_color="#EC7063", text_color="#FFFFFF", height=35, width=105)
    ...
    def load_puzzle_to_grid(self, grid_data, is_play_mode=False):
        n = len(grid_data)
        # Nếu size khác hiện tại thì rebuild
        if n != self.current_n:
            self.rebuild_grid(n)
    ...
    def get_grid_data(self):
        ...
        for r in range(self.current_n):
            ...
            for c in range(self.current_n):
                o_nhap_lieu = self.cac_o_nhap[(r, c)]
                val = o_nhap_lieu.get()
                if val == "": row_data.append(0)
                elif val.isdigit(): row_data.append(int(val))
                else: raise ValueError(f"Giá trị không hợp lệ tại ô ({r+1}, {c+1})")
    ...
    def cap_nhat_o_visual(self, data: dict, puzzle_data: list):
        ...
        if action == "try":
            ...
            for (nr, nc) in data["neighbors"]:
                # Kiểm tra xem ô này có trong lưới không 
